...
- **ERROR (blocks publish)** — channel-harm only: tags Telegram would 400 on, unbalanced markup, an empty body. In `mode='fallback'` it also errors on `DIGEST_LIMIT` floods, quant/serving artifacts, and stale release dates. Remaining non-https hrefs (`javascript:`, `data:`, relative) error in fallback. `http://` hrefs are rewritten to `https://` before the gate (2026-08-13: two aggregator http links from Parallel.ai research blocked the day's post). On the curated path a block sends an ops alert, records a `blocked` run, and falls through to the fallback.
...
- **Coverage and quality expansion (2026-08-20)** — sequenced plan in [`superpowers/specs/2026-08-20-coverage-and-quality-expansion.md`](./superpowers/specs/2026-08-20-coverage-and-quality-expansion.md). **Shipped:** persist published digest bodies; HF org casing (`inclusionAI`, `MiniMaxAI`, `BAAI`, `ByteDance-Seed`); `is_significant_release` uses `KNOWN_ORGS`; `sig_org_map` keys off the name slug. Still open: SPECIALIZED modality allowlist, OpenRouter recency, HF Daily Papers + lab feeds. Candidate queue: [`source-candidates.md`](./source-candidates.md).
